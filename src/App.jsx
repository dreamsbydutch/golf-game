...
export default function App(){
	...
	const [groupId,setGroupId]=useState(()=>localStorage.getItem(GROUP_KEY)),[screen,setScreen]=useState('entry'),[cardGroup,setCardGroup]=useState(null)
	const [holeIndex,setHoleIndex]=useState(()=>Math.max(0,Math.min(17,+localStorage.getItem(holeKey(localStorage.getItem(GROUP_KEY)))||0))),[draft,setDraft]=useState([]),[saving,setSaving]=useState(false),[confirmBlanks,setConfirmBlanks]=useState(false),[settings,setSettings]=useState(null)
	...
	if(screen==='admin')return <Admin groups={settings??game.groups} setGroups={setSettings} onBack={()=>setScreen('entry')} onSave={async()=>{await saveSettings({groups:settings});setScreen('entry')}} onReset={async()=>{await resetRound({});setHoleIndex(0);if(groupId)localStorage.setItem(holeKey(groupId),'0');setScreen('entry')}}/>
	if(!groupId||!group)return <GroupPicker groups={game.groups} onSelect={selectGroup} onAdmin={()=>{setSettings(structuredClone(game.groups));setScreen('admin')}}/>

	const hole=holes[holeIndex],submitted=group.submittedHoles[holeIndex]
	const setValue=(index,value,advance=false)=>{setDraft(values=>values.map((v,i)=>i===index?value:v));if(advance&&index<draft.length-1)setTimeout(()=>inputRefs.current[index+1]?.focus(),0)}
	const doSubmit=async()=>{const filled=draft.map(v=>v||'X');setSaving(true);await submitHole({groupId:group.id,holeIndex,scores:filled});setSaving(false);setConfirmBlanks(false);if(holeIndex<17){const next=holeIndex+1;setHoleIndex(next);localStorage.setItem(holeKey(group.id),String(next))}}
	...
	return <main className="shell">
		...
		<Leaderboard results={results} groups={game.groups} onOpen={setCardGroup}/>
		...
		{cardGroup&&<Scorecard group={game.groups.find(g=>g.id===cardGroup)} onClose={()=>setCardGroup(null)}/>}
	</main>
}
...
function Leaderboard({results,groups,onOpen}){
	...
	const players=groups.flatMap(group=>group.type==='individual'?group.players.map(player=>({player,group,...roundForPlayer(player,group)})):[]).sort((a,b)=>a.thru&&b.thru?a.toPar-b.toPar:b.thru-a.thru)
	...
	return <section className="leaderboard">
		...
		{view==='teams'?teams.map((r,i)=><button className="standing" key={r.group.id} onClick={()=>onOpen(r.group.id)}><span>{String(i+1).padStart(2,'0')}</span><div><b>{r.group.name}</b><small>{r.thru?`Thru ${r.thru}`:'Not started'}</small></div><strong>{r.thru?relative(r.toPar):'—'}</strong><i>›</i></button>):players.map((r,i)=><div className="standing player-standing" key={r.player.id}><span>{String(i+1).padStart(2,'0')}</span><div><b>{r.player.name}</b><small>{r.group.name} · {r.thru?`Thru ${r.thru}`:'Not started'}</small></div><strong>{r.thru?relative(r.toPar):'—'}</strong><i /> </div>)}
	</section>
}
function GroupPicker({groups,onSelect,onAdmin}){return <main className="picker"><span className="brand">THREESOME</span><div><p className="eyebrow">Match day</p><h1>Who are you scoring?</h1><p>Choose once. This phone will remember your group.</p></div><section>{groups.map(g=><button key={g.id} onClick={()=>onSelect(g.id)}><span>{g.type==='scramble'?'SCRAMBLE':'INDIVIDUAL'}</span><strong>{g.name}</strong><small>{g.type==='scramble'?'Emma · Stacey · Tracey':g.players.map(p=>p.name).join(' · ')}</small><i>→</i></button>)}</section><button className="organizer-link" onClick={onAdmin}>Organizer settings</button></main>}
function Scorecard({group,onClose}){const round=roundFor(group);return <div className="modal" role="dialog"><section><header><div><p className="eyebrow">Full scorecard</p><h2>{group.name}</h2></div><button onClick={onClose}>×</button></header><div className="card-total"><strong>{round.thru?relative(round.toPar):'—'}</strong><span>Net · thru {round.thru}</span></div><div className="scorecard-grid"><b>Hole</b><b>Par</b><b>Team net</b>{holes.map((h,i)=>{const r=resultFor(group,i);return <div className="scorecard-row" key={h.number}><span>{h.number}</span><span>{h.par}</span><strong>{r.value??'—'}</strong></div>})}</div></section></div>}
function Admin({groups,setGroups,onBack,onSave,onReset}){const patchGroup=(id,patch)=>setGroups(gs=>gs.map(g=>g.id===id?{...g,...patch}:g));return <main className="shell"><header><button className="back" onClick={onBack}>← Match</button><span className="brand">ORGANIZER</span></header><section className="settings-head"><p className="eyebrow">Match setup</p><h1>Players & handicaps</h1><p>Course handicaps are applied using each hole’s stroke index.</p></section><div className="settings">{groups.map(g=><section key={g.id}><label><span>Group name</span><input value={g.name} onChange={e=>patchGroup(g.id,{name:e.target.value})}/></label>{g.type==='scramble'?<label><span>Team handicap</span><input type="number" min="0" value={g.handicap} onChange={e=>patchGroup(g.id,{handicap:+e.target.value})}/></label>:g.players.map(p=><div className="player-setting" key={p.id}><input value={p.name} onChange={e=>patchGroup(g.id,{players:g.players.map(x=>x.id===p.id?{...x,name:e.target.value}:x)})}/><label><span>HCP</span><input type="number" min="0" value={p.handicap} onChange={e=>patchGroup(g.id,{players:g.players.map(x=>x.id===p.id?{...x,handicap:+e.target.value}:x)})}/></label></div>)}</section>)}<section className="danger"><h2>New round</h2><p>Keep this setup and erase all submitted scores.</p><button onClick={()=>window.confirm('Erase every score and start a new round? This cannot be undone.')&&onReset()}>Start new round</button></section></div><div className="save"><button onClick={onSave}>Save match setup</button></div></main>}
