...
export default function App(){
	const game=useQuery(api.gameState.get),submitHole=useMutation(api.gameState.submitGroupHole),resetRound=useMutation(api.gameState.resetRound)
	...
	const [holeIndex,setHoleIndex]=useState(()=>Math.max(0,Math.min(17,+localStorage.getItem(holeKey(localStorage.getItem(GROUP_KEY)))||0))),[draft,setDraft]=useState([]),[saving,setSaving]=useState(false),[confirmBlanks,setConfirmBlanks]=useState(false),[adminUnlocked,setAdminUnlocked]=useState(false)
	...
	if(screen==='admin'&&!adminUnlocked)return <AdminLogin onBack={()=>setScreen('entry')} onUnlock={()=>setAdminUnlocked(true)}/>
	if(screen==='admin')return <Admin groups={game.groups} onBack={()=>setScreen('entry')} onReset={async()=>{await resetRound({password:'Dutchy!4'});setHoleIndex(0);if(groupId)localStorage.setItem(holeKey(groupId),'0');setScreen('entry')}}/>
	if(!groupId||!group)return <GroupPicker groups={game.groups} onSelect={selectGroup} onAdmin={()=>setScreen('admin')}/>
	...
	return <main className="shell">
		<header><button className="group-switch" onClick={()=>setGroupId(null)}><span>Scoring</span>{group.name}⌄</button><button className="menu" onClick={()=>setScreen('admin')} aria-label="Organizer settings">•••</button></header>
...
function AdminLogin({onBack,onUnlock}){const [password,setPassword]=useState(''),[error,setError]=useState(false);const submit=e=>{e.preventDefault();if(password==='Dutchy!4')onUnlock();else setError(true)};return <main className="admin-login"><button className="back" onClick={onBack}>← Match</button><form onSubmit={submit}><span className="brand">ORGANIZER</span><p className="eyebrow">Protected area</p><h1>Enter password</h1><label><span>Password</span><input type="password" autoFocus value={password} onChange={e=>{setPassword(e.target.value);setError(false)}}/></label>{error&&<p className="login-error">Incorrect password</p>}<button type="submit">Unlock organizer tools</button></form></main>}
function Admin({groups,onBack,onReset}){return <main className="shell"><header><button className="back" onClick={onBack}>← Match</button><span className="brand">ORGANIZER</span></header><section className="settings-head"><p className="eyebrow">Match setup</p><h1>Players & handicaps</h1><p>This match configuration is read-only.</p></section><div className="settings readonly-settings">{groups.map(g=><section key={g.id}><span className="setting-label">{g.type==='scramble'?'Scramble group':'Individual group'}</span><h2>{g.name}</h2>{g.type==='scramble'?<div className="readonly-player"><span>Team handicap</span><strong>{g.handicap}</strong></div>:g.players.map(p=><div className="readonly-player" key={p.id}><span>{p.name}</span><strong>HCP {p.handicap}</strong></div>)}</section>)}<section className="danger"><h2>Reset scores</h2><p>Keep this setup and erase all submitted scores.</p><button onClick={()=>window.confirm('Erase every score and start a new round? This cannot be undone.')&&onReset()}>Start new round</button></section></div></main>}
