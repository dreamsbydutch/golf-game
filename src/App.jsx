...
export default function App(){
	...
	return <main className="shell">
		...
		<Leaderboard results={results} onOpen={setCardGroup}/>
...
function Leaderboard({results,onOpen}){return <section className="leaderboard"><div className="leader-head"><div><p className="eyebrow">Live leaderboard</p><h2>{[...results].filter(r=>r.thru).sort((a,b)=>a.toPar-b.toPar)[0]?.group.name??'Round ready'}</h2></div><span>NET</span></div>{[...results].sort((a,b)=>a.thru&&b.thru?a.toPar-b.toPar:b.thru-a.thru).map((r,i)=><button className="standing" key={r.group.id} onClick={()=>onOpen(r.group.id)}><span>0{i+1}</span><div><b>{r.group.name}</b><small>{r.thru?`Thru ${r.thru}`:'Not started'}</small></div><strong>{r.thru?relative(r.toPar):'—'}</strong><i>›</i></button>)}</section>}
